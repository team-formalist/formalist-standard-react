...
var _separator = require('./blocks/separator');
...
var defaults = {
  blockFormatters: ['unstyled', 'header-one', 'header-two', 'unordered-list-item', 'ordered-list-item', 'blockquote', 'pullquote', 'code-block', 'separator'],
  blockSet: {
    atomic: {
    ...
    handleKeyCommand: function handleKeyCommand(command, _ref4) {
      ...
      if (command === commands.DELETE_BLOCK) {
        ...
        if (_selection2.isCollapsed()) {
          setEditorState((0, _utils.removeAtomicBlock)(selectedAtomicBlockKey, _editorState2, false));
          return true;
        }
      } else if (command === commands.BACKSPACE_UNEDITABLE_BLOCK) {
        setEditorState(removeBlockBeforeCurrent(getEditorState()));
        return true;
      } else if (command === 'delete') {
        var _editorState3 = getEditorState();
        var contentState = _editorState3.getCurrentContent();
        ...
        if (_selection3.isCollapsed() && _selection3.getAnchorOffset() === selectedBlock.getLength()) {
          ...
          var blockToCheckKey = (0, _utils.getNextBlockKey)(selectedBlockKey, _editorState3);

          if (blockToCheckKey && contentState.getBlockForKey(blockToCheckKey).getType() === 'atomic') {
            setEditorState((0, _utils.removeAtomicBlock)(blockToCheckKey, _editorState3, false));
            return true;
          }
        }
      } else if (command === 'backspace') {
        var _editorState4 = getEditorState();
        var _selection4 = _editorState4.getSelection();
        if (_selection4.isCollapsed() && _selection4.getAnchorOffset() === 0) {
          var _contentState2 = _editorState4.getCurrentContent();
          // Check if the _next_ block is an atomic block
          var _blockToCheckKey = (0, _utils.getPreviousBlockKey)(_selection4.getAnchorKey(), _editorState4);
          if (_blockToCheckKey && _contentState2.getBlockForKey(_blockToCheckKey).getType() === 'atomic') {
            setEditorState((0, _utils.removeAtomicBlock)(_blockToCheckKey, _editorState4, true));
            return true;
          }
        }
      }
