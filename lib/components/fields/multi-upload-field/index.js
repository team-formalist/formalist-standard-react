...
var MultiUploadField = _react2.default.createClass({
  ...
  propTypes: {
    ...
    bus: _react2.default.PropTypes.object,
  ...
  */
  addToUploadQueue: function addToUploadQueue(id) {
    var bus = this.props.bus;
    var uploadQueue = this.state.uploadQueue;
    ...
    if (uploadQueue.length === 1) {
      bus.emit(_formalistCompose.events.internal.FIELD_BUSY, this.instanceId);
    }
  },
  ...
  */
  removeFromUploadQueue: function removeFromUploadQueue(id) {
    var bus = this.props.bus;

    var uploadQueue = this.state.uploadQueue.slice();
    ...
    if (uploadQueue.length === 0) {
      bus.emit(_formalistCompose.events.internal.FIELD_IDLE, this.instanceId);
    }
  },
  ...
  renderFiles: function renderFiles(files) {
    ...
    var isSortable = this.state.uploadQueue.length === 0;
