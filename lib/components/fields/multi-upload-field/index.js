...
var MultiUploadField = _react2.default.createClass({
  ...
  normaliseFileExport: function normaliseFileExport(obj) {
    return obj.hasOwnProperty('fileAttributes') ? obj : obj.fileAttributes;
  },
  ...
  onChange: function onChange(files) {
    ...
    var permittedFileTypeRegex = attributes.permittedFileTypeRegex;
    var permittedFileTypeMessage = attributes.permittedFileTypeMessage;
    var maxFileSize = attributes.maxFileSize;
    var maxFileSizeMessage = attributes.maxFileSizeMessage;
    ...
    var allFiles = this.props.attributes.multiple || this.props.multiple ? this.state.files.concat(uploadingFiles) : uploadingFiles;
  ...
  renderPreviewItem: function renderPreviewItem(fileObject, index) {
    ...
    var hasThumbnail = (0, _utils.hasImageFormatType)(name);
  ...
  renderDefaultTemplate: function renderDefaultTemplate(fileObject, index) {
    ...
    var hasThumbnail = thumbnail_url != null || (0, _utils.hasImageFormatType)(name);
  ...
  renderFiles: function renderFiles(files) {
    ...
    var allFiles = files.map(function (fileObject, index) {
      if (fileObject.file) {
        isSortable = false;
        return _this3.renderPreviewItem(fileObject, index);
      } else {
        var template = _this3.customComponentExists(config, render_uploaded_as) ? _this3.renderCustomTemplate(fileObject, index, config, render_uploaded_as) : _this3.renderDefaultTemplate(fileObject, index);
        return template;
      }
    });
