...
var MultiUploadField = _react2.default.createClass({
  ...
  abortUploadRequest: function abortUploadRequest(file) {
    (0, _attacheUpload.deleteXHRRequest)(file.uid);
  },
  ...
  onChange: function onChange(files) {
    ...
    var allFiles = uploadingFiles.concat(this.state.files);
    this.setState({
      files: allFiles
    });
  ...
  renderUploadedItem: function renderUploadedItem(fileObject, index) {
  ...
  renderFiles: function renderFiles(files) {
    ...
    var allFiles = files.map(function (file) {
      if (file.file) {
        isSortable = false;
        return _this3.renderPreviewItem(file);
      } else {
        return _this3.renderUploadedItem(file);
      }
    });
