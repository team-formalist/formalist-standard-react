...
class Popout extends React.Component {
      ...
      position = {
        ...
        top: Math.round(
          referencePosition.top +
            scrollY +
            vertOffset +
            (referencePosition.height / 2 - arrowVertPosition)
        )
      };
  ...
  render() {
    ...
    let containerClassNames = classNames(styles.container, [
      `${styles["container" + capitalize(placement)]}`
    ]);
    let arrowClassNames = classNames(styles.containerArrow, [
      `${styles["containerArrow" + capitalize(placement)]}`
    ]);
