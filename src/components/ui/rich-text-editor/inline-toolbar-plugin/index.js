...
const formattersMapping = [
  {
    ...
    style: "BOLD",
    icon:
      '<svg width="16" height="16" xmlns="http://www.w3.org/2000/svg"><path d="M0 0v2c1.1 0 2 .9 2 2v8c0 1.1-.9 2-2 2v2h11c2.76 0 5-2.24 5-5 0-2-1.18-3.7-2.88-4.5.54-.68.88-1.56.88-2.5 0-2.2-1.78-4-4-4h-10zm6 2h2c1.1 0 2 .9 2 2s-.9 2-2 2h-2v-4zm0 6h3c1.66 0 3 1.34 3 3s-1.34 3-3 3h-3v-6z"/></svg>'
  },
  {
    command: "italic",
    label: "Italic",
    style: "ITALIC",
    icon:
      '<svg width="14" height="16" xmlns="http://www.w3.org/2000/svg"><polygon points="3.98071795 2.21767396e-16 3.98071795 1.99750312 7.24130602 1.99750312 7.12128437 2.25717853 3.12056282 12.2446941 2.44044015 14.0024969 0 14.0024969 0 16 10.0018039 16 10.0018039 14.0024969 6.74121583 14.0024969 6.86123747 13.7428215 10.861959 3.75530587 11.5420817 1.99750312 13.9825218 1.99750312 13.9825218 0 3.98071795 0"/></svg>'
  },
  {
    command: "code",
    label: "Code",
  ...
  },
  {
    ...
    style: "UNDERLINE",
    icon:
      '<svg width="14" height="16" xmlns="http://www.w3.org/2000/svg"><path d="M2 0v8c0 2.2 2.24 4 5 4h1c2.2 0 4-1.8 4-4v-8h-2v8c0 1.1-.9 2-2 2s-2-.9-2-2v-8h-4zm-2 14v2h14v-2h-14z"/></svg>'
  },
  {
...
export default function inlineToolbarPlugin(options = {}) {
  ...
  allowedFormatters = allowedFormatters.concat(
    additionalFormatters.map(formatter => formatter.command)
  );

  // Reconcile the lists of formatters and entities from various sources
  const formatters = formattersMapping
    .concat(additionalFormatters)
    .filter(item => allowedFormatters.indexOf(item.command) > -1);
  const entities = entitiesMapping
    .concat(additionalEntities)
    .filter(entity => allowedEntities.indexOf(entity.type.toLowerCase()) > -1);
