...
class BlockItems extends React.Component {
  ...
  renderItemsGroups = itemsGroups => {
    ...
    return itemsGroups.map(group => {
      ...
      const iconWrapperClassNames = classNames(styles.iconWrapper, {
      ...
      return (
        <button
          ...
          onClick={e => {
            e.preventDefault();
            const nextBlockType = getNextBlockTypeToApply(
              currentBlockType,
              types
            );
